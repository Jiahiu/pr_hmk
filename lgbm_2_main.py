...
train["marital"] = train["marital"].map(
    {"unknown": 0, "single": 1, "married": 2, "divorced": 3}
)
test["marital"] = test["marital"].map(
    {"unknown": 0, "single": 1, "married": 2, "divorced": 3}
)
...
train["day_of_week"] = train["day_of_week"].map(
    {"mon": 0, "tue": 1, "wed": 2, "thu": 3, "fri": 4}
)
test["day_of_week"] = test["day_of_week"].map(
    {"mon": 0, "tue": 1, "wed": 2, "thu": 3, "fri": 4}
)

# 使用map方法对'poutcome'列进行编码
train["poutcome"] = train["poutcome"].map(
...
model_lgb.fit(X_train, y_train)  # 在训练集上训练模型

y_pred_proba = model_lgb.predict(X_train)
# 将概率转换为类别预测（0或1）
